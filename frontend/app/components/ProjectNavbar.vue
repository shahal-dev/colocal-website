...
<template>
  <div v-if="hasSlug" class="w-full sticky top-0 z-60 bg-white backdrop-blur shadow-sm border-b border-gray-200">
    <nav class="mx-auto flex items-center gap-2 px-4 md:px-6 lg:px-8 overflow-x-auto hide-scrollbar"
      style="overflow-y: visible !important">
      <NuxtLink v-if="projectName === 'COLOCAL'" :to="basePath"
        class="flex items-center shrink-0 mr-2 md:mr-16 px-1 py-0.5 h-6">
        <img src="~/assets/logos/COLOCAL-logo.png" alt="CoLocal logo" class="h-16 w-auto" />
      </NuxtLink>
    ...
    <Teleport to="body">
      <div v-show="dropdownOpen"
        class="fixed w-52 bg-white rounded-lg border border-slate-200 shadow-lg transition-opacity duration-200" :style="{
          top: dropdownPosition.top,
          left: dropdownPosition.left,
          zIndex: 9999,
        }" @mouseenter="dropdownOpen = true" @mouseleave="closeDropdown">
        <div class="py-1">
          <NuxtLink :to="`${basePath}/about`"
            class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 border-b border-slate-200">
            About {{ projectName }}
          </NuxtLink>
          <NuxtLink :to="`${basePath}/team`" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
            Our Team
          </NuxtLink>
        </div>
      </div>
