...
const countryPublications = computed(() => {
  return (
    publications.value?.filter(
      (p) => p.country?.name.toLowerCase() === countryData.value?.name.toLowerCase()
    ) || []
  );
});
...
fill-rule="evenodd"
      ...
      <!-- Carousel -->
      <div
v-if="currentCountryImages.length > 0" :class="[
...
v-for="member in teamMembers" :key="member.id"
            ...
            <div
              ...
              <img
v-if="member.avatar?.url" :src="member.avatar.url" :alt="member.name"
...
v-else
      ...
      <div v-if="countryPublications.length > 0" class="w-full mb-16">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <article
v-for="p in countryPublications" :key="p.id"
            ...
            <NuxtLink :to="`/resource-hub/${p.documentId || p.id}`" class="flex flex-col h-full">
              ...
              <div class="mt-auto pt-4 border-t border-gray-100">
                ...
                <div class="flex flex-wrap gap-1.5">
                  <span
v-for="(tag, idx) in (p.tags || []).slice(0, 3)" :key="idx"
      ...
      <div v-if="countryNewsEvents.length > 0" class="w-full mb-16">
        ...
        <div class="grid grid-cols-1 gap-6">
          <article
v-for="e in countryNewsEvents" :key="e.id"
...
stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div v-if="countryEducationTrainings.length > 0" class="w-full mb-8">
        ...
        <div class="grid grid-cols-1 gap-6">
          <article
v-for="e in countryEducationTrainings" :key="e.id"
            class="border border-gray-200 rounded-xl bg-white p-0 overflow-hidden hover:shadow-md transition-shadow">
            <NuxtLink
