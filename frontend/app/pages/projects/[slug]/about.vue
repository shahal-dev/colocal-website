...
<template>
  <div class="flex flex-col items-center w-full justify-center">
    ...
    <template v-else>
      ...
      <!-- About content -->
      <div class="w-full max-w-4xl mx-auto px-4 md:px-0 py-12 mb-10 text-justify">
        <section class="mb-12">
          <p class="text-xs font-semibold text-green-700 uppercase tracking-wide mb-2 text-center md:text-left">
            {{ projectName }}
          </p>
          <h1
        ...
        <section class="mb-12">
          ...
          <div v-if="project?.objectives" class="space-y-4">
            <ul class="list-disc pl-6 space-y-4 text-gray-700 leading-relaxed marker:text-green-600 text-lg">
              <li v-for="objective in project?.objectives || []" :key="project?.objectives.indexOf(objective)">
                {{ objective.objective }}
              </li>
            </ul>
          </div>
