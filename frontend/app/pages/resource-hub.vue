  ...
  <div class="flex flex-col items-center w-full justify-center">
    ...
    <template v-else>
      ...
      <section class="w-full max-w-6xl mx-auto py-10 px-4 md:px-0">
        ...
        <!-- Tabs -->
        <nav class="mt-5 flex flex-wrap items-center justify-center gap-2" aria-label="Filter resources by type">
          <button v-for="tab in tabs" :key="tab" type="button"
            class="px-4 py-2 rounded-sm border text-sm transition-colors" :class="activeTab === tab
        ...
        <div class="mt-6 space-y-4">
          <NuxtLink v-for="item in visible" :key="`${item.kind}-${item.id}`" :to="`/resource-hub/${item.id}`"
            ...
            <div class="block p-4 md:p-5">
              <div v-if="activeTab === 'Publications' && item.pubType" class="flex items-center gap-2 text-xs mb-2">
                <span class="inline-block px-2 py-0.5 rounded border border-blue-200 text-blue-700 bg-blue-50">
                  {{ item.pubType }}
                </span>
              </div>
